...
function register_rest_routes() {
	...
	register_rest_route(
		...
		array(
			'methods'             => 'POST',
			'callback'            => __NAMESPACE__ . '\\get_pull_content_post',
			'args'                => get_pull_content_post_args(),
			'permission_callback' => __NAMESPACE__ . '\\get_pull_content_post_permission',
		)
	);
}

/**
 * REST API arguments for the pull-content/post endpoint.
 *
 * @return array Arguments for endpoint.
 */
function get_pull_content_post_args() {
	return array(
		'id'             => array(
			'validate_callback' => function( $value ) {
				// Wrap call to `is_numeric()` in a closure to avoid PHP throwing a warning.
		...
		'post_status'    => array(
			'validate_callback' => function( $value ) {
				// Wrap call to `is_string()` in a closure to avoid PHP throwing a warning.
 ...
 */
function get_pull_content_post( $request ) {
	$post_id = $request['id'];
	$args    = array();
...
/**
 * Check if a user has permission to pull content of a particular post.
 *
 * @since x.x.x
 *
 * @param  WP_REST_Request $request Request object.
 ...
 */
function get_pull_content_post_permission( $request ) {
